...
func newLTable(acap int, hcap int) *LTable {
	...
	if hcap != 0 {
		tb.strdict = smap.NewSyncMMap[string, LValue]()
	}
	return tb
}
...
func (tb *LTable) RawSetString(key string, value LValue) {
	...
	if tb.keys == nil {
		tb.keys = []LValue{}
		tb.k2i = smap.NewSyncMMap[string, int]()
	}

	if value == LNil {
		// TODO tb.keys and tb.k2i should also be removed
		tb.strdict.Delete(key)
	} else {
		tb.strdict.Store(key, value)
		lkey := LString(key)
		if _, ok := tb.k2i.Load(lkey.String()); !ok {
			tb.k2i.Store(lkey.String(), len(tb.keys))
...
func (tb *LTable) RawSetH(key LValue, value LValue) {
	...
	if tb.dict == nil {
		tb.dict = smap.NewSyncMMap[string, LValue]()
	}
	if tb.keys == nil {
		tb.keys = []LValue{}
		tb.k2i = smap.NewSyncMMap[string, int]()
	...
	if value == LNil {
		// TODO tb.keys and tb.k2i should also be removed
		tb.strdict.Delete(key.String())
	} else {
		tb.dict.Store(key.String(), value)
		if _, ok := tb.k2i.Load(key.String()); !ok {
			tb.k2i.Store(key.String(), len(tb.keys))
...
func (tb *LTable) RawGet(key LValue) LValue {
	...
	case LString:
		...
		if ret, ok := tb.strdict.Load(v.String()); ok {
			return ret
		}
		return LNil
	}
	...
	if v, ok := tb.dict.Load(key.String()); ok {
		return v
	}
	return LNil
}
...
func (tb *LTable) RawGetH(key LValue) LValue {
	if s, sok := key.(LString); sok {
		...
		if v, vok := tb.strdict.Load(s.String()); vok {
			return v
		}
		return LNil
	}
...
func (tb *LTable) RawGetString(key string) LValue {
	...
	if v, vok := tb.strdict.Load(key); vok {
		return v
	}
	return LNil
}
...
func (tb *LTable) ForEach(cb func(LValue, LValue)) {
	...
	if tb.strdict != nil {
		tb.strdict.Range(func(k string, v LValue) bool {
			if v != LNil {
				cb(LString(k), v)
			}
	...
	if tb.dict != nil {
		tb.dict.Range(func(k string, v LValue) bool {
			if v != LNil {
				cb(LString(k), v)
			}
...
func (tb *LTable) Next(key LValue) (LValue, LValue) {
	...
	if init || key != LNumber(0) {
		if kv, ok := key.(LNumber); ok && isInteger(kv) && int(kv) >= 0 && kv < LNumber(MaxArrayIndex) {
			...
			if tb.array == nil || index == len(tb.array) {
				if (tb.dict == nil || tb.dict.Len() == 0) && (tb.strdict == nil || tb.strdict.Len() == 0) {
					return LNil, LNil
				}
				key = tb.keys[0]
	...
	for i := tb.k2i.Get(key.String()) + 1; i < len(tb.keys); i++ {
		key := tb.keys[i]
		if v := tb.RawGetH(key); v != LNil {
			return key, v
		}
