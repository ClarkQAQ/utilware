...
func getMetatable(L *lua.LState, vtype reflect.Type) *lua.LTable {
	...
	if v := config.regular.Get(vtype.String()); v != nil {
		return v
	}
	...
	case reflect.Map:
		...
		mt.RawSetString("__index", L.NewFunction(mapLockIndex(indexLock)))
		mt.RawSetString("__newindex", L.NewFunction(mapLockNewIndex(indexLock)))
	...
	config.regular.Store(vtype.String(), mt)
	return mt
}
