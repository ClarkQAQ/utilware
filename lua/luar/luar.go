...
func lValueToReflect(L *lua.LState, v lua.LValue, hint reflect.Type, tryConvertPtr *bool) (reflect.Value, error) {
	visited := smap.NewSyncMap[*lua.LTable, reflect.Value]()
	return lValueToReflectInner(L, v, hint, visited, tryConvertPtr)
}

func lValueToReflectInner(L *lua.LState, v lua.LValue, hint reflect.Type, visited *smap.SyncMap[*lua.LTable, reflect.Value], tryConvertPtr *bool) (reflect.Value, error) {
	...
	case *lua.LTable:
		if existing := visited.Get(converted); existing.IsValid() {
			return existing, nil
		}
