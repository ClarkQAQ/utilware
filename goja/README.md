...
[![GoDoc](https://godoc.org/utilware/goja?status.svg)](https://godoc.org/utilware/goja)
 ...
 * Sourcemaps.
 * Some ES6 functionality, still work in progress, see https://utilware/goja/milestone/1?closed=1
...
(see https://utilware/goja/issues/250 and https://utilware/goja/issues/199 for more details).
...
You can find some benchmarks [here](https://utilware/goja/issues/2).
...
There is a [separate project](https://utilware/goja_nodejs) aimed at providing some NodeJS functionality,
...
See https://utilware/goja/milestone/1 for more details.
...
for ETAs. Features that are open in the [milestone](https://utilware/goja/milestone/1) are either in progress
...
--------------------
Any Go value can be passed to JS using Runtime.ToValue() method. See the method's [documentation](https://godoc.org/utilware/goja#Runtime.ToValue) for more details.

Exporting Values from JS
------------------------
A JS value can be exported into its default Go representation using Value.Export() method.

Alternatively it can be exported into a specific Go variable using [Runtime.ExportTo()](https://godoc.org/utilware/goja#Runtime.ExportTo) method.
...
- Using [AssertFunction()](https://godoc.org/utilware/goja#AssertFunction):
```go
vm := New()
_, err := vm.RunString(`
function sum(a, b) {
...
// Output: 42
```
- Using [Runtime.ExportTo()](https://godoc.org/utilware/goja#Runtime.ExportTo):
```go
const SCRIPT = `
function f(param) {
    return +param + 2;
...
dealing with a 3rd party library, you can use a [FieldNameMapper](https://godoc.org/utilware/goja#FieldNameMapper):
...
There are two standard mappers: [TagFieldNameMapper](https://godoc.org/utilware/goja#TagFieldNameMapper) and
[UncapFieldNameMapper](https://godoc.org/utilware/goja#UncapFieldNameMapper), or you can use your own implementation.

Native Constructors
-------------------

In order to implement a constructor function in Go use `func (goja.ConstructorCall) *goja.Object`.
See [Runtime.ToValue()](https://godoc.org/utilware/goja#Runtime.ToValue) documentation for more details.

Regular Expressions
-------------------

Goja uses the embedded Go regexp library where possible, otherwise it falls back to [regexp2](https://utilware/dep/regexp2).
...
There is a [separate project](https://utilware/goja_nodejs) aimed at providing some of the NodeJS functionality.
