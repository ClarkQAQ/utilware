...
import (
	...
	"utilware/dep/x/crypto/openpgp/packet"
)
...
func TestRevokedUserID(t *testing.T) {
	...
	// [ultimate] (1)  Golang Gopher <no-reply@ccms.com>
	// [ revoked] (2)  Golang Gopher <revoked@ccms.com>
	...
	if identityName, expectedName := identities[0].Name, "Golang Gopher <no-reply@ccms.com>"; identityName != expectedName {
...
func TestKeyWithRevokedSubKey(t *testing.T) {
	...
	//  uid   Golang Gopher <no-reply@ccms.com>
	...
	identity := keys[0].Identities["Golang Gopher <no-reply@ccms.com>"]
...
func TestNewEntityWithPreferredHash(t *testing.T) {
	...
	entity, err := NewEntity("Golang Gopher", "Test Key", "no-reply@ccms.com", c)
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewEntityWithoutPreferredHash(t *testing.T) {
	entity, err := NewEntity("Golang Gopher", "Test Key", "no-reply@ccms.com", nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewEntityCorrectName(t *testing.T) {
	...
	want := "Golang Gopher (Test Key) <no-reply@ccms.com>"
	if got != want {
