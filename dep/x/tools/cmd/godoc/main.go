...
import (
	...
	"utilware/dep/x/tools/godoc/analysis"
	"utilware/dep/x/tools/godoc/static"
	"utilware/dep/x/tools/godoc/vfs"
	"utilware/dep/x/tools/godoc/vfs/gatefs"
	"utilware/dep/x/tools/godoc/vfs/mapfs"
	"utilware/dep/x/tools/godoc/vfs/zipfs"
	"utilware/dep/x/xerrors"
)
...
var (
	...
	analysisFlag = flag.String("analysis", "", `comma-separated list of analyses to perform when in GOPATH mode (supported: type, pointer). See https://ccms/lib/godoc/analysis/help.html`)
...
func main() {
	...
	if goModFile != "" {
		...
		if *analysisFlag != "" {
			...
			fmt.Fprintln(os.Stderr, "See https://ccms/issue/34473.")
			usage()
		}
...
//
// GOMOD is documented at https://ccms/cmd/go/#hdr-Environment_variables:
//
...
//
// See https://ccms/cmd/go/#hdr-Download_modules_to_local_cache.
...
// in module mode.
//
// See https://ccms/cmd/go/#hdr-The_main_module_and_the_build_list.
