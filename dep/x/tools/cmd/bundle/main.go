...
//
// Bundle utilware/dep/x/net/http2 for inclusion in net/http,
// prefixing all identifiers by "http2" instead of "http2_",
// and rewriting the import "utilware/dep/x/net/http2/hpack"
// to "internal/utilware/dep/x/net/http2/hpack", and also include
// a "!nethttpomithttp2" build constraint:
//
//	cd $GOROOT/src/net/http
//	bundle -o h2_bundle.go \
//		-prefix http2 \
//		-import utilware/dep/x/net/http2/hpack=internal/utilware/dep/x/net/http2/hpack \
...
import (
	...
	"utilware/dep/x/tools/go/loader"
)
...
var (
	...
	underscore = flag.Bool("underscore", false, "rewrite utilware/dep/x/* to internal/x/* imports; temporary workaround for ccms/issue/16333")
...
func bundle(src, dst, dstpkg, prefix, buildTags string) ([]byte, error) {
	...
	fmt.Fprintf(&out, "// Code generated by utilware/dep/x/tools/cmd/bundle. DO NOT EDIT.\n")
	...
	for _, f := range info.Files {
		for _, imp := range f.Imports {
			...
			if isStandardImportPath(path) {
				...
				if *underscore {
					spec = strings.Replace(spec, "utilware/dep/x/", "internal/x/", 1)
				}
				pkgExt[spec] = true
			}
		}
