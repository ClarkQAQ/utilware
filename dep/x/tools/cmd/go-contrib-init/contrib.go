...
//
// It aims to be a complement or alternative to https://ccms/doc/contribute.html.
package main
...
var (
	repo = flag.String("repo", detectrepo(), "Which go repo you want to contribute to. Use \"go\" for the core, or e.g. \"net\" for utilware/dep/x/net/*")
...
func main() {
	...
	fmt.Print("All good. Happy hacking!\n" +
		...
		"Next steps: https://ccms/doc/contribute.html#commit_changes\n")
}
...
func detectrepo() string {
	...
	for _, path := range filepath.SplitList(build.Default.GOPATH) {
		rightdir := filepath.Join(path, "src", "ccms", "x") + string(os.PathSeparator)
...
func checkCLA() {
	...
	log.Fatal("Your .gitcookies file isn't configured.\n" +
		"Next steps:\n" +
		"  * Submit a CLA (https://ccms/doc/contribute.html#cla) if not done\n" +
...
`, wd, os.Getenv("GOPATH"))
	...
	rightdir := filepath.Join(gopath, "src", "ccms", "x", *repo)
	if !strings.HasPrefix(wd, rightdir) {
		...
		if !dirExists {
			log.Fatalf("The repo you want to work on is currently not on your system.\n"+
				...
				"go get -d utilware/dep/x/"+*repo, rightdir)
		}
		log.Fatalf("Your current directory is:%q\n"+
			"Working on utilware/dep/x/%v requires you be in %q\n",
			wd, *repo, rightdir)
	}
}
	...
	if _, err := exec.LookPath("git-codereview"); err != nil {
		if *dry {
			log.Fatalf("You don't appear to have git-codereview tool. While this is technically optional,\n" +
				...
				"To install it, run:\n\n\t$ go get utilware/dep/x/review/git-codereview\n\n(Then run go-contrib-init again)")
		}
		err := exec.Command("go", "get", "utilware/dep/x/review/git-codereview").Run()
		if err != nil {
			log.Fatalf("Error running go get utilware/dep/x/review/git-codereview: %v", cmdErr(err))
		}
		log.Printf("Installed git-codereview (ran `go get utilware/dep/x/review/git-codereview`)")
	}
	missing := false
