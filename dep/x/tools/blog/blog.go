...
package blog // import "utilware/dep/x/tools/blog"
...
import (
	...
	"utilware/dep/x/tools/blog/atom"
	"utilware/dep/x/tools/present"
)

var (
	validJSONPFunc = regexp.MustCompile(`(?i)^[a-z_][a-z0-9_.]*$`)
	// used to serve relative paths when ServeLocalLinks is enabled.
	ccmsOrgAbsLinkReplacer = strings.NewReplacer(
		`href="https://ccms/pkg`, `href="/pkg`,
		`href="https://ccms/cmd`, `href="/cmd`,
	)
)
...
type Config struct {
	...
	PlayEnabled     bool
	ServeLocalLinks bool // rewrite ccms/{pkg,cmd} links to host-less, relative paths.
}
...
func (s *Server) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if s.cfg.ServeLocalLinks {
		...
		_, err = ccmsOrgAbsLinkReplacer.WriteString(w, buf.String())
	} else {
