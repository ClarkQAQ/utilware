	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			testConfig{
				...
				t.assertProcessEquals("ccms/fake", "x.go", nil, options, tt.out)
			})
...
`
	...
	}.processTest(t, "ccms/fake", "x.go", nil, nil, want)
}
...
`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			}.processTest(t, "ccms/fake", "x.go", nil, options, tt.out)
		})
	}
...
`
	...
	}.processTest(t, "ccms/fake", "myotherpackage/toformat.go", nil, nil, want)
}
	...
	testConfig{
		...
		module: packagestest.Module{
			...
			Files: fm{
				...
				"../../.goimportsignore": "ccms/fake/x/y/mypkg\n",
			},
		},
	}.processTest(t, "ccms/fake", "myotherpkg/toformat.go", nil, nil, want)
}
...
import "utilware/dep/x/net/http2/hpack"
...
`
	testConfig{
		...
		module: packagestest.Module{
			Name: "foo.com",
			Files: fm{
				"vendor/utilware/dep/x/net/http2/hpack/huffman.go": "package hpack\nfunc HuffmanDecode() { }\n",
				"bar/x.go": input,
			},
		},
...
func (c testConfig) test(t *testing.T, fn func(*goimportTest)) {
	...
	for _, exporter := range packagestest.All {
		t.Run(exporter.Name(), func(t *testing.T) {
			...
			if it.env.GOROOT == "" {
				// packagestest clears out GOROOT to work around https://ccms/issue/32849,
