...
executable lives at utilware/dep/x/tools/cmd/godoc.
...
```
godoc -templates=$GOPATH/src/utilware/dep/x/tools/godoc/static -http=:6060
```
...
2) Run `go generate utilware/dep/x/tools/godoc/static` so `static/static.go` picks
up the change.

3) Run `go install utilware/dep/x/tools/cmd/godoc` so the compiled `godoc` binary
picks up the change.
