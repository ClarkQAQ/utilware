...
func testCgoNoSyntax(t *testing.T, exporter packagestest.Exporter) {
	...
	// ccms/cl/214943 for more details.
	if !hasGoBuild() {
		t.Skip("this test can't run on platforms without go build. See discussion on ccms/cl/214943 for more details.")
	}
	...
	for _, mode := range modes {
		t.Run(fmt.Sprint(mode), func(t *testing.T) {
			...
			pkgs, err := packages.Load(exported.Config, "ccms/fake/c")
			if err != nil {
				t.Fatal(err)
			}
			if len(pkgs) != 1 {
