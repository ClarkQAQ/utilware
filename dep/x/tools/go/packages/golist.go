...
import (
	...
	"utilware/dep/x/tools/go/internal/packagesdriver"
...
func (state *golistState) adhocPackage(pattern, query string) (*driverResponse, error) {
	...
	if len(response.Packages) == 1 {
		// ccms/go#33482: If this is a file= query for ad-hoc packages where
...
func (state *golistState) createDriverResponse(words ...string) (*driverResponse, error) {
	...
	for dec := json.NewDecoder(buf); dec.More(); {
		...
		// Work around https://ccms/issue/33157:
		...
		if old, found := seen[p.ImportPath]; found {
			...
			// See ccms/issue/36188 for more details.
		...
		// Work around https://ccms/issue/28749:
		...
		if p.Error != nil {
			msg := strings.TrimSpace(p.Error.Err) // Trim to work around ccms/issue/32363.
			// Address ccms/issue/35964 by appending import stack to error message.
...
func (state *golistState) invokeGo(verb string, args ...string) (*bytes.Buffer, error) {
	...
	if err := cmd.Run(); err != nil {
		...
		isPkgPathRune := func(r rune) bool {
			// From https://ccms/ref/spec#Import_declarations:
		...
		if len(stderr.String()) > 0 && strings.HasPrefix(stderr.String(), "# ") {
			...
			// Treat pkg-config errors as a special case (ccms/issue/36770).
			if strings.HasPrefix(msg, "pkg-config") {
				return stdout, nil
			}
		}

		// This error only appears in stderr. See ccms/cl/166398 for a fix in go list to show
		...
		// Workaround for an instance of ccms/issue/26755: go list -e  will return a non-zero exit
	...
	// them in the Package.Error JSON (see https://ccms/issue/26319).
