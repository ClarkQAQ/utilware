...
import (
	...
	"utilware/dep/x/tools/internal/packagesinternal"
	"utilware/dep/x/tools/internal/testenv"
)

// testCtx is canceled when the test binary is about to time out.
//
// If https://ccms/issue/28135 is accepted, uses of this variable in test
...
func testLoadImportsGraph(t *testing.T, exporter packagestest.Exporter) {
	exported := packagestest.Export(t, exporter, []packagestest.Module{{
		...
		Files: map[string]interface{}{
			...
			"b/b.go":             `package b; import ("ccms/fake/a"; _ "container/list"); var B = a.A`,
			"c/c.go":             `package c; import (_ "ccms/fake/b"; _ "unsafe")`,
			"c/c2.go":            "// +build ignore\n\n" + `package c; import _ "fmt"`,
			"subdir/d/d.go":      `package d`,
			"subdir/d/d_test.go": `package d; import _ "math/bits"`,
			"subdir/d/x_test.go": `package d_test; import _ "ccms/fake/subdir/d"`, // TODO(adonovan): test bad import here
			"subdir/e/d.go":      `package e`,
			"e/e.go":             `package main; import _ "ccms/fake/b"`,
			"e/e2.go":            `package main; import _ "ccms/fake/c"`,
			"f/f.go":             `package f`,
		}}})
	defer exported.Cleanup()
	exported.Config.Mode = packages.LoadImports
	initial, err := packages.Load(exported.Config, "ccms/fake/c", "ccms/fake/subdir/d", "ccms/fake/e")
	if err != nil {
		t.Fatal(err)
	}
...
* ccms/fake/e
* ccms/fake/subdir/d
* ccms/fake/subdir/d [ccms/fake/subdir/d.test]
* ccms/fake/subdir/d.test
* ccms/fake/subdir/d_test [ccms/fake/subdir/d.test]
  math/bits
  unsafe
  ccms/fake/b -> container/list
  ccms/fake/b -> ccms/fake/a
  ccms/fake/c -> ccms/fake/b
  ccms/fake/c -> unsafe
  ccms/fake/e -> ccms/fake/b
  ccms/fake/e -> ccms/fake/c
  ccms/fake/subdir/d [ccms/fake/subdir/d.test] -> math/bits
  ccms/fake/subdir/d.test -> ccms/fake/subdir/d [ccms/fake/subdir/d.test]
  ccms/fake/subdir/d.test -> ccms/fake/subdir/d_test [ccms/fake/subdir/d.test]
  ccms/fake/subdir/d.test -> os (pruned)
  ccms/fake/subdir/d.test -> testing (pruned)
  ccms/fake/subdir/d.test -> testing/internal/testdeps (pruned)
  ccms/fake/subdir/d_test [ccms/fake/subdir/d.test] -> ccms/fake/subdir/d [ccms/fake/subdir/d.test]
`[1:]
...
`[1:]
	...
	for _, test := range []struct {
		...
		{"ccms/fake/a", "a", "package", "a.go"},
		{"ccms/fake/b", "b", "package", "b.go"},
		{"ccms/fake/c", "c", "package", "c.go"}, // c2.go is ignored
		{"ccms/fake/e", "main", "command", "e.go e2.go"},
		{"container/list", "list", "package", "list.go"},
		{"ccms/fake/subdir/d", "d", "package", "d.go"},
		{"ccms/fake/subdir/d.test", "main", "command", "0.go"},
	...
	if initial, err := packages.Load(exported.Config, exported.File("ccms/fake", "c/c.go")); len(initial) == 0 {
	...
	{
		// "..." (subdirectory)
		initial, err = packages.Load(exported.Config, "ccms/fake/subdir/...")
		if err != nil {
			t.Fatal(err)
		}
		graph, _ = importGraph(initial)
...
func testLoadImportsTestVariants(t *testing.T, exporter packagestest.Exporter) {
	exported := packagestest.Export(t, exporter, []packagestest.Module{{
		Name: "ccms/fake",
		Files: map[string]interface{}{
			"a/a.go":       `package a; import _ "ccms/fake/b"`,
			"b/b.go":       `package b`,
			"b/b_test.go":  `package b`,
			"b/bx_test.go": `package b_test; import _ "ccms/fake/a"`,
		}}})
	defer exported.Cleanup()
	exported.Config.Mode = packages.LoadImports
	exported.Config.Tests = true

	initial, err := packages.Load(exported.Config, "ccms/fake/a", "ccms/fake/b")
	if err != nil {
		t.Fatal(err)
	}
	...
	wantGraph := `
* ccms/fake/a
  ccms/fake/a [ccms/fake/b.test]
* ccms/fake/b
* ccms/fake/b [ccms/fake/b.test]
* ccms/fake/b.test
* ccms/fake/b_test [ccms/fake/b.test]
  ccms/fake/a -> ccms/fake/b
  ccms/fake/a [ccms/fake/b.test] -> ccms/fake/b [ccms/fake/b.test]
  ccms/fake/b.test -> ccms/fake/b [ccms/fake/b.test]
  ccms/fake/b.test -> ccms/fake/b_test [ccms/fake/b.test]
  ccms/fake/b.test -> os (pruned)
  ccms/fake/b.test -> testing (pruned)
  ccms/fake/b.test -> testing/internal/testdeps (pruned)
  ccms/fake/b_test [ccms/fake/b.test] -> ccms/fake/a [ccms/fake/b.test]
`[1:]
...
func TestLoadAbsolutePath(t *testing.T) {
	...
	initial, err := packages.Load(exported.Config, filepath.Dir(exported.File("ccms/gopatha", "a/a.go")), filepath.Dir(exported.File("ccms/gopathb", "b/b.go")))
	if err != nil {
	...
	sort.Strings(got)
	want := []string{"ccms/gopatha/a", "ccms/gopathb/b"}
...
func TestVendorImports(t *testing.T) {
	exported := packagestest.Export(t, packagestest.GOPATH, []packagestest.Module{{
		Name: "ccms/fake",
		Files: map[string]interface{}{
			"a/a.go":          `package a; import _ "b"; import _ "ccms/fake/c";`,
			"a/vendor/b/b.go": `package b; import _ "ccms/fake/c"`,
			"c/c.go":          `package c; import _ "b"`,
			"c/vendor/b/b.go": `package b`,
		}}})
	defer exported.Cleanup()
	exported.Config.Mode = packages.LoadImports
	initial, err := packages.Load(exported.Config, "ccms/fake/a", "ccms/fake/c")
	if err != nil {
		t.Fatal(err)
	}
	...
	wantGraph := `
* ccms/fake/a
  ccms/fake/a/vendor/b
* ccms/fake/c
  ccms/fake/c/vendor/b
  ccms/fake/a -> ccms/fake/a/vendor/b
  ccms/fake/a -> ccms/fake/c
  ccms/fake/a/vendor/b -> ccms/fake/c
  ccms/fake/c -> ccms/fake/c/vendor/b
`[1:]
	if graph != wantGraph {
		t.Errorf("wrong import graph: got <<%s>>, want <<%s>>", graph, wantGraph)
	}
	...
	for _, test := range []struct {
		...
		{"ccms/fake/a", "b:ccms/fake/a/vendor/b ccms/fake/c:ccms/fake/c"},
		{"ccms/fake/c", "b:ccms/fake/c/vendor/b"},
		{"ccms/fake/a/vendor/b", "ccms/fake/c:ccms/fake/c"},
		{"ccms/fake/c/vendor/b", ""},
	} {
...
func testConfigDir(t *testing.T, exporter packagestest.Exporter) {
	...
	defer exported.Cleanup()
	aDir := filepath.Dir(exported.File("ccms/fake", "a/a.go"))
	bDir := filepath.Dir(exported.File("ccms/fake", "b/b.go"))
	...
	for _, test := range []struct {
		...
		{dir: bDir, pattern: "ccms/fake/a", want: `"a"`},
		{dir: bDir, pattern: "ccms/fake/b", want: `"b"`},
		{dir: bDir, pattern: "./a", fails: true},
		{dir: bDir, pattern: "./b", fails: true},
		{dir: baseDir, pattern: "ccms/fake/a", want: `"a"`},
		{dir: baseDir, pattern: "ccms/fake/b", want: `"b"`},
		{dir: baseDir, pattern: "./a", want: `"a"`},
		{dir: baseDir, pattern: "./b", want: `"b"`},
		{dir: aDir, pattern: "ccms/fake/a", want: `"a"`},
		{dir: aDir, pattern: "ccms/fake/b", want: `"b"`},
...
func testConfigFlags(t *testing.T, exporter packagestest.Exporter) {
	...
	exported := packagestest.Export(t, exporter, []packagestest.Module{{
		...
		Files: map[string]interface{}{
			// package a
			"a/a.go": `package a; import _ "ccms/fake/a/b"`,
			"a/b.go": `// +build tag
...
package b`,
	...
	for _, test := range []struct {
		...
		{`ccms/fake/a`, []string{}, "a.go", "a.go"},
		{`ccms/fake/a`, []string{`-tags=tag`}, "a.go b.go c.go", "a.go b.go"},
		{`ccms/fake/a`, []string{`-tags=tag2`}, "a.go c.go", "a.go"},
		{`ccms/fake/a`, []string{`-tags=tag tag2`}, "a.go b.go c.go d.go", "a.go b.go"},
	} {
		exported.Config.Mode = packages.LoadImports
...
func testLoadTypes(t *testing.T, exporter packagestest.Exporter) {
	...
	exported := packagestest.Export(t, exporter, []packagestest.Module{{
		Name: "ccms/fake",
		Files: map[string]interface{}{
			"a/a.go": `package a; import "ccms/fake/b"; import "ccms/fake/c"; const A = "a" + b.B + c.C`,
	...
	initial, err := packages.Load(exported.Config, "ccms/fake/a")
	if err != nil {
		t.Fatal(err)
	}
...
func testLoadTypesBits(t *testing.T, exporter packagestest.Exporter) {
	exported := packagestest.Export(t, exporter, []packagestest.Module{{
		Name: "ccms/fake",
		Files: map[string]interface{}{
			"a/a.go": `package a; import "ccms/fake/b"; const A = "a" + b.B`,
			"b/b.go": `package b; import "ccms/fake/c"; const B = "b" + c.C`,
			"c/c.go": `package c; import "ccms/fake/d"; const C = "c" + d.D`,
			"d/d.go": `package d; import "ccms/fake/e"; const D = "d" + e.E`,
			"e/e.go": `package e; import "ccms/fake/f"; const E = "e" + f.F`,
...
* ccms/fake/c
  ...
  ccms/fake/f
  ccms/fake/a -> ccms/fake/b
  ccms/fake/b -> ccms/fake/c
  ccms/fake/c -> ccms/fake/d
  ccms/fake/d -> ccms/fake/e
  ccms/fake/e -> ccms/fake/f
`[1:]
	if graph != wantGraph {
		t.Errorf("wrong import graph: got <<%s>>, want <<%s>>", graph, wantGraph)
	}
	...
	aA := constant(all["ccms/fake/a"], "A")
	if aA == nil {
		t.Fatalf("a.A: got nil")
	}
	if got, want := fmt.Sprintf("%v %v", aA, aA.Val()), `const ccms/fake/a.A untyped string "abcdef"`; got != want {
...
`[1:]
	...
	for _, test := range []struct {
		...
		{"ccms/fake/a", true, true},   // source package
		{"ccms/fake/b", true, true},   // source package because depends on initial package
		{"ccms/fake/c", true, true},   // source package
		{"ccms/fake/d", false, true},  // export data package
		{"ccms/fake/e", false, false}, // export data package
		{"ccms/fake/f", false, false}, // export data package
	} {
...
func testLoadDiamondTypes(t *testing.T, exporter packagestest.Exporter) {
	...
	exported := packagestest.Export(t, exporter, []packagestest.Module{{
		Name: "ccms/fake",
		Files: map[string]interface{}{
			"a/a.go": `package a; import ("ccms/fake/b"; "ccms/fake/c"); var _ = b.B == c.C`,
			"b/b.go": `package b; import "ccms/fake/d"; var B d.D`,
			"c/c.go": `package c; import "ccms/fake/d"; var C d.D`,
...
func testLoadSyntaxError(t *testing.T, exporter packagestest.Exporter) {
	...
	exported := packagestest.Export(t, exporter, []packagestest.Module{{
		...
		Files: map[string]interface{}{
			...
			"c/c.go": `package c; import "ccms/fake/d"; const C = "c" + d.D`,
			"d/d.go": `package d; import "ccms/fake/e"; const D = "d" + e.E`,
			"e/e.go": `package e; import "ccms/fake/f"; const E = "e" + f.F + 1`, // type error
			"f/f.go": `package f; const F = "f"`,
		}}})
	defer exported.Cleanup()
	...
	for _, test := range []struct {
		...
		{"ccms/fake/a", true, true},
		{"ccms/fake/b", true, true},
		{"ccms/fake/c", true, true},
		{"ccms/fake/d", true, true},
		{"ccms/fake/e", true, true},
		{"ccms/fake/f", false, false},
	} {
		p := all[test.id]
		if p == nil {
			t.Errorf("missing package: %s", test.id)
	...
	if got, want := aA.String(), `const ccms/fake/a.A invalid type`; got != want {
		t.Errorf("a.A: got %s, want %s", got, want)
	}
}
...
func testOverlay(t *testing.T, exporter packagestest.Exporter) {
	...
	for i, test := range []struct {
		...
		{map[string][]byte{exported.File("ccms/fake", "c/c.go"): []byte(`package c; const C = "C"`)}, `"abC"`, nil},
		{map[string][]byte{exported.File("ccms/fake", "b/b.go"): []byte(`package b; import "ccms/fake/c"; const B = "B" + c.C`)}, `"aBc"`, nil},
		// Overlay with an existing file in an existing package adding a new import.
		{map[string][]byte{exported.File("ccms/fake", "b/b.go"): []byte(`package b; import "ccms/fake/d"; const B = "B" + d.D`)}, `"aBd"`, nil},
		// Overlay with an existing file in an existing package.
		{map[string][]byte{exported.File("ccms/fake", "c/c.go"): []byte(`package c; import "net/http"; const C = http.MethodGet`)}, `"abGET"`, nil},
		// Overlay with a new file in an existing package.
		{map[string][]byte{
			exported.File("ccms/fake", "c/c.go"):                                               []byte(`package c;`),
			filepath.Join(filepath.Dir(exported.File("ccms/fake", "c/c.go")), "c_new_file.go"): []byte(`package c; const C = "Ç"`)},
			`"abÇ"`, nil},
		// Overlay with a new file in an existing package, adding a new dependency to that package.
		{map[string][]byte{
			exported.File("ccms/fake", "c/c.go"):                                               []byte(`package c;`),
			filepath.Join(filepath.Dir(exported.File("ccms/fake", "c/c.go")), "c_new_file.go"): []byte(`package c; import "ccms/fake/d"; const C = "c" + d.D`)},
			`"abcd"`, nil},
	} {
...
func testOverlayDeps(t *testing.T, exporter packagestest.Exporter) {
	...
	exported.Config.Overlay = map[string][]byte{exported.File("ccms/fake", "c/c.go"): []byte(`package c; import "net/http"; const C = http.MethodGet`)}
	...
	pkgs, err := packages.Load(exported.Config, fmt.Sprintf("file=%s", exported.File("ccms/fake", "c/c.go")))
	if err != nil {
		t.Error(err)
	}

	// Find package ccms/fake/c
	sort.Slice(pkgs, func(i, j int) bool { return pkgs[i].ID < pkgs[j].ID })
	pkgc := pkgs[0]
	if pkgc.ID != "ccms/fake/c" {
		t.Errorf("expected first package in sorted list to be \"ccms/fake/c\", got %v", pkgc.ID)
	}

	// Make sure ccms/fake/c imports net/http, as per the overlay.
...
func testNewPackagesInOverlay(t *testing.T, exporter packagestest.Exporter) {
	...
	dir := filepath.Dir(filepath.Dir(exported.File("ccms/fake", "a/a.go")))
	...
	for _, test := range []struct {
		...
		{"one_file",
			map[string][]byte{
				filepath.Join(dir, "e", "e.go"): []byte(`package e; import "ccms/fake/a"; const E = "e" + a.A`)},
			`"eabc"`},
		{"multiple_files_same_package",
			map[string][]byte{
				filepath.Join(dir, "e", "e.go"):      []byte(`package e; import "ccms/fake/a"; const E = "e" + a.A + underscore`),
				filepath.Join(dir, "e", "e_util.go"): []byte(`package e; const underscore = "_"`),
			},
			`"eabc_"`},
		{"multiple_files_two_packages",
			map[string][]byte{
				filepath.Join(dir, "e", "e.go"):      []byte(`package e; import "ccms/fake/f"; const E = "e" + f.F + underscore`),
				filepath.Join(dir, "e", "e_util.go"): []byte(`package e; const underscore = "_"`),
		...
		{"multiple_files_three_packages",
			map[string][]byte{
				filepath.Join(dir, "e", "e.go"):      []byte(`package e; import "ccms/fake/f"; const E = "e" + f.F + underscore`),
				filepath.Join(dir, "e", "e_util.go"): []byte(`package e; const underscore = "_"`),
				filepath.Join(dir, "f", "f.go"):      []byte(`package f; import "ccms/fake/g"; const F = "f" + g.G`),
				filepath.Join(dir, "g", "g.go"):      []byte(`package g; const G = "g"`),
			},
			`"efg_"`},
		{"multiple_files_four_packages",
			map[string][]byte{
				filepath.Join(dir, "e", "e.go"):      []byte(`package e; import "ccms/fake/f"; import "ccms/fake/h"; const E = "e" + f.F + h.H + underscore`),
				filepath.Join(dir, "e", "e_util.go"): []byte(`package e; const underscore = "_"`),
				filepath.Join(dir, "f", "f.go"):      []byte(`package f; import "ccms/fake/g"; const F = "f" + g.G`),
				filepath.Join(dir, "g", "g.go"):      []byte(`package g; const G = "g"`),
		...
		{"multiple_files_four_packages_again",
			map[string][]byte{
				...
				filepath.Join(dir, "e", "e_util.go"): []byte(`package e; const underscore = "_"`),
				filepath.Join(dir, "f", "f.go"):      []byte(`package f; import "ccms/fake/g"; const F = "f" + g.G`),
				filepath.Join(dir, "g", "g.go"):      []byte(`package g; import "ccms/fake/h"; const G = "g" + h.H`),
				filepath.Join(dir, "h", "h.go"):      []byte(`package h; const H = "h"`),
			},
			`"efgh_"`},
		{"main_overlay",
			map[string][]byte{
				filepath.Join(dir, "e", "main.go"): []byte(`package main; import "ccms/fake/a"; const E = "e" + a.A; func main(){}`)},
			`"eabc"`},
	} {
...
func testOverlayNewPackageAndTest(t *testing.T, exporter packagestest.Exporter) {
	...
	dir := filepath.Dir(exported.File("ccms/fake", "foo.txt"))
...
func TestOverlayModFileChanges(t *testing.T) {
	...
	// mod1 has a dependency on utilware/dep/x/xerrors.
	...
	require (
		utilware/dep/x/xerrors v0.0.0-20190717185122-a985d3407aa7
	)
	`), 0775); err != nil {
		t.Fatal(err)
	}
...
func TestOverlayGOPATHVendoring(t *testing.T) {
	...
	exported.Config.Logf = t.Logf
	exported.Config.Overlay = map[string][]byte{
		exported.File("ccms/fake", "user/user.go"): []byte(`package user; import "vendor.com/foo"; var x = foo.X`),
	}
	initial, err := packages.Load(exported.Config, "ccms/fake/user")
	if err != nil {
		t.Fatal(err)
	}
	user := initial[0]
...
func testAbsoluteFilenames(t *testing.T, exporter packagestest.Exporter) {
	exported := packagestest.Export(t, exporter, []packagestest.Module{{
		...
		Files: map[string]interface{}{
			"a/a.go":          `package a; const A = 1`,
			"b/b.go":          `package b; import ("ccms/fake/a"; _ "errors"); var B = a.A`,
	...
	defer exported.Cleanup()
	exported.Config.Dir = filepath.Dir(filepath.Dir(exported.File("ccms/fake", "a/a.go")))
	...
	for _, test := range []struct {
		...
		// Import paths
		{"ccms/fake/a", "a.go"},
		{"ccms/fake/b/vendor/a", "a.go"},
		{"ccms/fake/b", "b.go"},
		{"ccms/fake/c", "c.go"},
		{"ccms/fake/subdir/d", "d.go"},
		{"ccms/fake/subdir/e", "d.go"},
		{"ccms/fake/e", "e.go e2.go"},
		{"ccms/fake/f", "f.go f.s"},
		// Relative paths
		{"./a", "a.go"},
		{"./b/vendor/a", "a.go"},
		{"./b", "b.go"},
...
func testContains(t *testing.T, exporter packagestest.Exporter) {
	exported := packagestest.Export(t, exporter, []packagestest.Module{{
		Name: "ccms/fake",
		Files: map[string]interface{}{
			"a/a.go": `package a; import "ccms/fake/b"`,
			"b/b.go": `package b; import "ccms/fake/c"`,
			"c/c.go": `package c`,
		}}})
	defer exported.Cleanup()
	bFile := exported.File("ccms/fake", "b/b.go")
	exported.Config.Mode = packages.LoadImports
...
func testContainsOverlay(t *testing.T, exporter packagestest.Exporter) {
	...
	defer exported.Cleanup()
	bOverlayFile := filepath.Join(filepath.Dir(exported.File("ccms/fake", "b/b.go")), "b_overlay.go")
	exported.Config.Mode = packages.LoadImports
...
func testContainsOverlayXTest(t *testing.T, exporter packagestest.Exporter) {
	...
	defer exported.Cleanup()
	bOverlayXTestFile := filepath.Join(filepath.Dir(exported.File("ccms/fake", "b/b.go")), "b_overlay_x_test.go")
	exported.Config.Mode = packages.NeedName | packages.NeedFiles | packages.NeedImports
	exported.Config.Overlay = map[string][]byte{bOverlayXTestFile: []byte(`package b_test; import "ccms/fake/b"`)}
...
func testContainsFallbackSticks(t *testing.T, exporter packagestest.Exporter) {
	...
	exported.Config.Mode = packages.LoadImports
	bFile := exported.File("ccms/fake", "b/b.go")
	initial, err := packages.Load(exported.Config, "ccms/fake/a", "file="+bFile)
	if err != nil {
		t.Fatal(err)
	}
...
func testJSON(t *testing.T, exporter packagestest.Exporter) {
	...
	exported := packagestest.Export(t, exporter, []packagestest.Module{{
		...
		Files: map[string]interface{}{
			"a/a.go": `package a; const A = 1`,
			"b/b.go": `package b; import "ccms/fake/a"; var B = a.A`,
			"c/c.go": `package c; import "ccms/fake/b" ; var C = b.B`,
			"d/d.go": `package d; import "ccms/fake/b" ; var D = b.B`,
		}}})
	defer exported.Cleanup()

	exported.Config.Mode = packages.LoadImports
	initial, err := packages.Load(exported.Config, "ccms/fake/c", "ccms/fake/d")
	if err != nil {
		t.Fatal(err)
	}
...
`[1:]
	...
	for i, want := range []*packages.Package{{
		...
		Imports: map[string]*packages.Package{
			"ccms/fake/a": {ID: "ccms/fake/a"},
		},
	}, {
		ID:   "ccms/fake/c",
		Name: "c",
		Imports: map[string]*packages.Package{
			"ccms/fake/b": {ID: "ccms/fake/b"},
		},
	}, {
		ID:   "ccms/fake/d",
		Name: "d",
...
`)
	...
	defer exported.Cleanup()
	driver := exported.File("ccms/fake", "bin/gopackagesdriver")
...
// package (specifically in this case of ccms/fake/a), and this triggered
...
func testBasicXTest(t *testing.T, exporter packagestest.Exporter) {
	...
	exported.Config.Mode = packages.LoadFiles
	exported.Config.Tests = true
	_, err := packages.Load(exported.Config, "ccms/fake/a")
	if err != nil {
		t.Fatal(err)
	}
}
...
func testReturnErrorWhenUsingGoFilesInMultipleDirectories(t *testing.T, exporter packagestest.Exporter) {
	...
	pkgs, err := packages.Load(exported.Config, exported.File("ccms/gopatha", "a/a.go"), exported.File("ccms/gopatha", "b/b.go"))
	if err != nil {
...
func testReturnErrorForUnexpectedDirectoryLayout(t *testing.T, exporter packagestest.Exporter) {
	...
	exported.Config.Dir = filepath.Dir(exported.File("ccms/gopatha", "a/testdata/a.go"))
...
func testMissingDependency(t *testing.T, exporter packagestest.Exporter) {
	...
	exported.Config.Mode = packages.LoadAllSyntax
	pkgs, err := packages.Load(exported.Config, "ccms/fake/a")
	if err != nil {
		t.Fatal(err)
	}
	if len(pkgs) != 1 && pkgs[0].PkgPath != "ccms/fake/a" {
		t.Fatalf("packages.Load: want [ccms/fake/a], got %v", pkgs)
	}
	if len(pkgs[0].Errors) == 0 {
		t.Errorf("result of Load: want package with errors, got none: %+v", pkgs[0])
	}
...
func testLoadTypesInfoWithoutNeedDeps(t *testing.T, exporter packagestest.Exporter) {
	...
	const expectedImport = "ccms/fake/b"
...
func testLoadWithNeedDeps(t *testing.T, exporter packagestest.Exporter) {
	exported := packagestest.Export(t, exporter, []packagestest.Module{{
		...
		Files: map[string]interface{}{
			"a/a.go": `package a; import _ "ccms/fake/b"`,
			"b/b.go": `package b; import _ "ccms/fake/c"`,
			"c/c.go": `package c`,
		}}})
	defer exported.Cleanup()
...
func testMultiplePackageVersionsIssue36188(t *testing.T, exporter packagestest.Exporter) {
	...
	pkgs, err := packages.Load(exported.Config, "ccms/fake/a", "ccms/fake/b")
	if err != nil {
		t.Fatal(err)
	}
	sort.Slice(pkgs, func(i, j int) bool { return pkgs[i].ID < pkgs[j].ID })
	...
	if pkgs[0].ID != "ccms/fake/a" && pkgs[1].ID != "ccms/fake/b" {
		t.Fatalf(`expected (sorted) IDs "ccms/fake/a" and "ccms/fake/b", got %q and %q`,
			pkgs[0].ID, pkgs[1].ID)
	}
	if pkgs[0].Errors == nil {
		t.Errorf(`expected error on package "ccms/fake/a", got none`)
	}
	if pkgs[1].Errors != nil {
		t.Errorf(`expected no errors on package "ccms/fake/b", got %v`, pkgs[1].Errors)
	}
	defer exported.Cleanup()
}
...
func testCycleImportStack(t *testing.T, exporter packagestest.Exporter) {
	exported := packagestest.Export(t, exporter, []packagestest.Module{{
		...
		Files: map[string]interface{}{
			"a/a.go": `package a; import _ "ccms/fake/b"`,
			"b/b.go": `package b; import _ "ccms/fake/a"`,
		}}})
	defer exported.Cleanup()
	...
	expected := "import cycle not allowed: import stack: [ccms/fake/a ccms/fake/b ccms/fake/a]"
