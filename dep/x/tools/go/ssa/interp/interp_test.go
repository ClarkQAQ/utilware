...
import (
	...
	"utilware/dep/x/tools/go/ssa/interp"
...
func run(t *testing.T, input string) bool {
	// The recover2 test case is broken when run against tip. See ccms/go#34089.
	...
	if filepath.Base(input) == "recover2.go" && strings.HasPrefix(runtime.Version(), "devel") {
		t.Skip("The recover2.go test is broken in tip. See ccms/issue/34089.")
	}
	...
	hint = fmt.Sprintf("To dump SSA representation, run:\n%% go build utilware/dep/x/tools/cmd/ssadump && ./ssadump -test -build=CFP %s\n", input)
	...
	hint = fmt.Sprintf("To trace execution, run:\n%% go build utilware/dep/x/tools/cmd/ssadump && ./ssadump -build=C -test -run --interp=T %s\n", input)
