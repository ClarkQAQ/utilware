...
//   g.Go(func() error { ... })
//
// Currently only "utilware/dep/x/sync/errgroup.Group()" is considered.
func goInvokes(info *types.Info, call *ast.CallExpr) ast.Expr {
	f := typeutil.StaticCallee(info, call)
	// Note: Currently only supports: utilware/dep/x/sync/errgroup.Go.
	if f == nil || f.Name() != "Go" {
		return nil
	}
	...
	if pkg.Path() != "utilware/dep/x/sync/errgroup" {
		return nil
	}
	return call.Args[0]
}
